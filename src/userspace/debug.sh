...
gu() {
	...
	i586-pc-TextOS-gdb -tui --eval-command "set confirm off" \
                      --eval-command "add-symbol-file $1 $(/usr/local/cross/bin/i586-pc-TextOS-objdump -x $1| perl -ne '/start address (0x\d{8})/ && print $1')" \
